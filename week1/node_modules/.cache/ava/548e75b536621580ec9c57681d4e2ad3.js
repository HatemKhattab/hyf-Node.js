...
function testCmd(state, ...methods) {
  test(`/${methods.join(', /')} returns ${state}`, async t => {
    ...
    for (const method of methods) try {
      response = await request(t.context.server).get(`/${method}`).timeout(TIMEOUT);
    } catch (err) {
    ...
    t.is(response.headers['content-type'], 'application/json', 'Expected content type to be application/json');
...
test('querying undefined URL returns 404 Not Found', async t => {
  let response;
  try {
    response = await request(t.context.server).get(`/random-bad-url`).timeout(TIMEOUT);
  } catch (err) {
    return t.fail(`Expected server to respond without an error, got: ${err.message}`);
  }
...
//# sourceMappingURL=/Users/hatemkhattab/Documents/projects/Node.js/week1/homework/node_modules/.cache/ava/548e75b536621580ec9c57681d4e2ad3.js.map